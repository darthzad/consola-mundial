...
# ==========================================
# 3. MOTOR DE DATOS (TIMEOUT A 60 SEGUNDOS)
# ==========================================
def obtener_datos_directos(url):
    try:
        # Aumentado a 60 segundos por solicitud
...
if menu == "📡 En vivo":
    ...
    with col_t2:
        # HORA CONFIGURADA PARA NICARAGUA
        tz_nicaragua = pytz.timezone('America/Managua')
        fecha_nicaragua = datetime.now(tz_nicaragua).strftime("%d %b %Y - %H:%M")
        st.markdown(f"<div style='background-color: #1f2937; padding: 10px 20px; border-radius: 20px; text-align: center; color: #9ca3af; font-size: 14px;'>📅 {fecha_nicaragua} (NIC)</div>", unsafe_allow_html=True)
    ...
    # HTML PEGADO A LA IZQUIERDA (Sin sangría para evitar que Streamlit lo haga código)
    tarjeta_html = f"""
